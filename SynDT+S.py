...
# pddlFile = r"domain\2.Nim\2.12 Odd-or-Even Nim\One-piled-odd-or-even-nim.pddl"  
...
# cover(term) the number of term covering pts
...
while(True):
    ...
    if str(e) != str(last_e):
        if Game["type"] == "normal" :
            ...
            con3 = And(Game["Constraint"], e,Not(Game["Terminal_Condition"]), Exists(varListY, And(global_transition_formula, e1)))
        ...
        if check1 == sat:
            # print("unsat con1")
            print("The constraint of winning formula is not valid and add countexamples, go to the next round of the learning process.")
...
def isTermSatExample(term,pt,output):
    ...
    if Game["var_num"] == 2:
        ...
        for i in range(1,len(term)):
            if eval(str(term[i]).replace('v2',str(pt[1])).replace('v3',str(pt[2])).replace('v1',str(pt[0]))) != output[i]:
                return False
        return True
    elif Game["var_num"] == 1:
        for i in range(1,len(term)):
        ...
        if DT != None:
            ...
            for path in ActPaths:
                ...
                for i  in range(1,len(concreteAct)): 
                    ...
                    action["precondition"] = eval(str(action["precondition"]).replace(str(action["action_parameter"][i-1]),"("+str(concreteAct[i])+")"))
                    action["transition_formula"] = eval(str(action["transition_formula"]).replace(str(action["action_parameter"][i-1]),"("+str(concreteAct[i])+")"))
                ...
                s.add(con)
                if s.check() == sat:
                    print("The constraint of winning strategy is not valid and add countexamples, go to the next round of the learning process.")
