                        ...
                        for choose2 in ExpSet:
                            if choose2['size'] == li-size1:
                                if termination_sign:
                                    ...
                                    sheet1.write(row, 2 , "time-out-"+time_out1+"s")
                        ...
                        for choose2 in ExpSet:
                            if choose2['size'] == li-size1:
                                if termination_sign:
                                    Thread1.cancel()
                                    print("Time out,about to exit the program")
                                    sheet1.write(row, 4 , "time-out-"+time_out1+"s")
                                    newwb.save(resultFile)
                                    sys.exit(0)
                                term = FunExg[i['Function_name']](choose1['Expression'], choose2['Expression'])
                                Goal1= []
...
winning_formula_Y = eval(str(winning_formula).replace("v2", "v2_next").replace("v3", "v3_next").replace("v1", "v1_next"))
...
for cover in refinement:
    print("----------------------test the splitting :", cover, "--------------------")
    s = Solver()
    s.add(cover)
    s.add(Game["Constraint"])
    if(s.check() == unsat):  
    ...
    for action in actions:
        ...
        while (True):
            ...
            if (s.check() == unsat):
                print("the semi ground acton",action["action_name"]+"("+str(e)+")","sat the splitting")
                winningStrategy.append([cover, action["action_name"]+"("+str(e)+")"])
                flagAct = True
                break
            else:
                num1 = 0
